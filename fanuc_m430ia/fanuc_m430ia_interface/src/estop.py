...
def estop_callback(msg):
    ...
    stopMoveGroup = rospy.Publisher('move_group/cancel', GoalID, queue_size=10)
    stopConveyor = rospy.Publisher('CONVEYORPOWER', ConveyorBeltState, queue_size=10)

    g = GoalID()
    g.stamp = rospy.Time.now()
    g.id = ''
    stopMoveGroup.publish(g)

    conveyor_state = ConveyorBeltState()
    conveyor_state.enabled = False
    conveyor_state.power = 0
    stopConveyor.publish(conveyor_state)
