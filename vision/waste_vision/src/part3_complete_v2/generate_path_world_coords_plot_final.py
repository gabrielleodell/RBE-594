...
# Image dimensions and world scaling
IMG_WIDTH = 1920
IMG_HEIGHT = 1080
WORLD_WIDTH_MM = 900
PIXEL_TO_MM = WORLD_WIDTH_MM / IMG_WIDTH

# Bin positions in pixels (top-left origin, camera-aligned)
bins_px = {
    0: np.array([0, 0]),
...
def path_from_order(order, bins, start_pos):
    ...
    path = []
    for obj in order:
        path.append((obj[0], obj[1]))                    # Pick
        path.append((bins[obj[2]][0], bins[obj[2]][1]))  # Place
        pos = bins[obj[2]]
    return path
...
def save_path_world_mm(path, out_file):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="path_world_mm.txt", help="Output XY path in mm")
    ...
    save_path_world_mm(path, args.output)

    print(f"Saved {len(path)} points (in mm) to {args.output}")
